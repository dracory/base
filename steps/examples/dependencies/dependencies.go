...
func NewDag() steps.DagInterface {
	...
	stepApplyDiscount := NewStepApplyDiscount().AddDependency(stepSetBasePrice)
	stepAddShipping := NewStepAddShipping().AddDependency(stepApplyDiscount)
	stepCalculateTax := NewStepCalculateTax().AddDependency(stepAddShipping)
